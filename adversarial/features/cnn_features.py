...
def extract_cnn_features(force: bool = False) -> pd.DataFrame:
    if CNN_FEATURES_PATH.exists() and not force:
        print(f"{CNN_FEATURES_PATH} already exists - not regenerating (pass force=True to override deliberately)")
        return pd.read_csv(CNN_FEATURES_PATH)
    ...
    out_df.to_csv(CNN_FEATURES_PATH, index=False)
    print(f"wrote {len(out_df)} rows -> {CNN_FEATURES_PATH}")
    return out_df
